...
cd /d "%~dp0"
"%PY%" -m PyInstaller --onefile --windowed --name "PreisAlarm" preis_alarm.py

if %errorlevel% neq 0 (
    echo.
    echo  [!] EXE-Erstellung fehlgeschlagen - App laeuft trotzdem ueber starten.bat
)

:: starten.bat mit pythonw (kein CMD-Fenster)
set "PYW=%PY:python.exe=pythonw.exe%"
(
    echo @echo off
    echo cd /d "%%~dp0"
    echo if exist "%PYW%" (
    echo     start "" "%PYW%" preis_alarm.py
    echo ) else (
    echo     powershell -WindowStyle Hidden -Command "Start-Process '%PY%' -ArgumentList 'preis_alarm.py' -WorkingDirectory '%%~dp0' -WindowStyle Hidden"
    echo )
) > starten_neu.bat
move /y starten_neu.bat starten.bat >nul
...
if exist "dist\PreisAlarm.exe" (
    ...
    echo.
    echo  [OK] starten.bat wurde aktualisiert - bitte diese Datei verwenden.
)
